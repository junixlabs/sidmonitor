...
import { useServiceEndpoints } from '../../hooks/useOutboundLogs'
import type { OutboundEndpointStats } from '../../types'

interface ServiceEndpointDetailsProps {
  serviceName: string
  onClose: () => void
  onViewLogs: (serviceName: string, endpointPattern: string, method: string) => void
}

export default function ServiceEndpointDetails({
  serviceName,
  onClose,
  onViewLogs,
}: ServiceEndpointDetailsProps) {
  const { data: endpoints = [], isLoading } = useServiceEndpoints(serviceName)
    ...
    <div className="bg-surface shadow rounded-lg">
      ...
      <div className="px-6 py-4">
        {isLoading ? (
          ...
          <EmptyState title="No endpoints found" description="No endpoints found for this service." />
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-border-subtle">
              ...
              <tbody className="bg-surface divide-y divide-border-subtle">
                {endpoints.map((endpoint: OutboundEndpointStats, index: number) => (
                  <tr key={`${endpoint.method}-${endpoint.endpoint_pattern}-${index}`} className="hover:bg-surface-secondary">
                    ...
                    <td className={cn(
                      ...
                      endpoint.failure_count > 0 ? 'text-status-danger' : 'text-status-success'
                    )}>
                      {endpoint.failure_count.toLocaleString()}
                    </td>
                    ...
                    <td className="px-4 py-3 text-sm text-right text-text-secondary">
                      {formatLatency(endpoint.p99_latency_ms)}
                    </td>
                    <td className="px-4 py-3 text-sm text-right text-text-muted">
                      {formatBytes(endpoint.avg_request_size)}
                    </td>
                    <td className="px-4 py-3 text-sm text-right text-text-muted">
                      {formatBytes(endpoint.avg_response_size)}
                    </td>
                    <td className="px-4 py-3 text-center">
                      <button
                        type="button"
                        onClick={() => onViewLogs(serviceName, endpoint.endpoint_pattern, endpoint.method)}
